...
for gnoise in noises:
    for i, (train, valid, test, contfeats, binfeats) in tqdm(enumerate(dataset.get_train_valid_test()), desc="Replications"):
        ...
        with tf.Graph().as_default():
            ...
            if pick_type == "top":
                ...
                train_filter = random.sample(range(0, num_train), int(top_per * num_train))
                test_filter = random.sample(range(0, num_test), int(top_per * num_test))

            else:
                train_filter = list(range(0, num_train))
