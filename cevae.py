...
from utils import fc_net, get_y0_y1, write_results, get_opt_args
import arguments

args = arguments.parse_args()
args = get_opt_args(args)
print(vars(args))
...
for i, (train, valid, test, contfeats, binfeats) in enumerate(dataset.get_train_valid_test()):
    ...
    if gnoise > 0:
        ...
        gnoise_test = np.random.normal(scale=gnoise,size=xte.shape)
    else:
        gnoise_train = np.zeros_like(xtr)
        gnoise_valid = np.zeros_like(xva)
        gnoise_test = np.zeros_like(xte)
    xtr += gnoise_train
    xva += gnoise_valid
    xte += gnoise_test
    # pad noise
    if pnoise:
        if task == 'ihdp':
            # gaussian noise
            pnoise_train = np.random.normal(scale=pnoise_scale,size=(xtr.shape[0], pnoise_size))
            pnoise_valid = np.random.normal(scale=pnoise_scale,size=(xva.shape[0], pnoise_size))
            pnoise_test  = np.random.normal(scale=pnoise_scale,size=(xte.shape[0], pnoise_size))
        else:
            # bernoulli noise
            pnoise_train = pnoise_scale * np.random.binomial(1,0.5,size=(xtr.shape[0], pnoise_size))
            pnoise_valid = pnoise_scale * np.random.binomial(1,0.5,size=(xva.shape[0], pnoise_size))
            pnoise_test  = pnoise_scale * np.random.binomial(1,0.5,size=(xte.shape[0], pnoise_size))
        xtr = np.concatenate([xtr, pnoise_train], axis=1)
        xva = np.concatenate([xva, pnoise_valid], axis=1)
        xte = np.concatenate([xte, pnoise_test] , axis=1)
    xalltr, talltr, yalltr = np.concatenate([xtr, xva], axis=0), np.concatenate([ttr, tva], axis=0), np.concatenate([ytr, yva], axis=0)
    x_dim = xalltr.shape[1]
    if task == 'jobs':
        ...
        if not test_only:
            for epoch in range(n_epoch):
                ...
                for j in range(n_iter_per_epoch):
                    ...
                    assert not np.any(np.isnan(avg_loss)), \
                        "During Processing {}, NaN Detected at Rep{}. Pass to next representaion".format(arg_info, i)
